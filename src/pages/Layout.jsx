...
export default function Layout({ children }) {
      ...
      try {
        ...
        unsub = onAuthStateChanged(auth, async (fbUser) => {
          ...
          try {
            ...
            const profile = snap.exists() ? { id: fbUser.uid, ...snap.data() } : { id: fbUser.uid };
            setCurrentUser(profile);

            const prefLang = profile?.settings?.language;
            if (prefLang) {
  ...
  // If authenticated but not onboarded → render page content without nav (same behavior as your 2nd code)
              ...
              {navigationItems.map((item, index) => (
                <SidebarMenuItem
                  ...
                  className={`rounded-lg ${
                    location.pathname === item.url ? "bg-green-100 text-green-700" : "hover:bg-gray-100"
                  }`}
                >
                      ...
                      {getMobileNavigationItems().more.map((item, index) => (
                        <Link
                          ...
                          className={`flex flex-col items-center justify-center p-2 rounded-xl transition-colors text-center ${
                            location.pathname === item.url ? "bg-green-100 text-green-700" : "hover:bg-white text-gray-700"
                          }`}
                        >
                          <div className={`flex items-center justify-center w-10 h-10 rounded-full mb-1.5 ${
                            location.pathname === item.url ? "bg-green-200" : "bg-gray-100"
                          }`}>
                            <item.icon className="w-5 h-5" />
                          </div>
