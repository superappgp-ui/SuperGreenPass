...
function buildUserDefaults({ email, full_name = '' }) {
  return {
    ...
    email,
    full_name,
    user_type: 'user', // changes if agent/tutor/school/vendor
    phone: '',
    country: '',
    ...
    settings: {
      ...
      notification_preferences: {
        ...
        application_updates: true, marketing_emails: false, session_reminders: true,
      },
    },
...
export default function Onboarding() {
  ...
  // 1) Wait for auth; if signed out -> send to Welcome
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      ...
      // 2) Ensure user profile exists, then load it
  ...
  const handleRoleSelect = (roleType) => { setSelectedRole(roleType); setCurrentStep(STEPS.BASIC_INFO); };
  const handleInputChange = (field, value) => setFormData((p) => ({ ...p, [field]: value }));
  const handleBasicInfoSubmit = () => setCurrentStep(STEPS.ROLE_SPECIFIC);
  const handleBack = () => {
    if (currentStep === STEPS.BASIC_INFO) setCurrentStep(STEPS.CHOOSE_ROLE);
    else if (currentStep === STEPS.ROLE_SPECIFIC) setCurrentStep(STEPS.BASIC_INFO);
  };
  ...
  // 3) Finalize onboarding: update Firestore, set onboarding_completed, go Dashboard
  const handleCompleteOnboarding = async () => {
    ...
    try {
      ...
      // Base updates common to all roles
      const updates = {
      ...
      // Map user_type based on selected role
      ...
      // Store role-specific sub-objects
      ...
      setTimeout(() => navigate(createPageUrl('Dashboard'), { replace: true }), 1500);
    } catch (e) {
      ...
      <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {ROLE_OPTIONS.map((role) => (
          <Card key={role.type} className="cursor-pointer hover:shadow-xl transition-all duration-300 border-2 hover:border-green-500 hover:scale-105 group"
                onClick={() => setSelectedRole(role.type) || setCurrentStep(STEPS.BASIC_INFO)}>
    ...
    return (
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <Button onClick={() => setCurrentStep(STEPS.ROLE_SPECIFIC)} className="flex-1" disabled={!(formData.full_name && formData.phone && formData.country)}>
  ...
  // loading gates (prevents ping-pong)
